...
public class DualSimManager {
	...
	private void updateOperatorDetails(Context mContext) {
		Log.d("\nSIM 1 :", this.getmSimOperatorName(0));
		Log.d("\nSIM 2 :", this.getmSimOperatorName(1));

		Log.d("\nOperator name 1 :", this.getNETWORK_OPERATOR_NAME(0));
		Log.d("\nOperator name 2 :", this.getNETWORK_OPERATOR_NAME(1));
	...
	class customTelephony{
		...
		public customTelephony(Context mContext) {
			try {
				...
				telephonyClassName = Utility.getPreferences(mContext, LocalConstants.DUAL_SIM_CLASS);
				SIM_VARINT = Utility.getPreferences(mContext, LocalConstants.SIM_VARINT);
				slotName_1 = Utility.getPreferences(mContext, LocalConstants.SIM_SLOT_NAME_1);
				slotName_2 = Utility.getPreferences(mContext, LocalConstants.SIM_SLOT_NAME_2);
				slotNumber_1 = Integer.parseInt(Utility.getPreferences(mContext, LocalConstants.SIM_SLOT_NUMBER_1));
				slotNumber_2 = Integer.parseInt(Utility.getPreferences(mContext, LocalConstants.SIM_SLOT_NUMBER_2));
				fetchClassInfo();
		...
		public void fetchClassInfo() {
			try {
				...
				Utility.setPreferences(mContext, LocalConstants.DUAL_SIM_CLASS, telephonyClassName);
				Utility.setPreferences(mContext, LocalConstants.SIM_VARINT, SIM_VARINT);
				Utility.setPreferences(mContext, LocalConstants.SIM_SLOT_NAME_1, slotName_1);
				Utility.setPreferences(mContext, LocalConstants.SIM_SLOT_NAME_2, slotName_2);
				Utility.setPreferences(mContext, LocalConstants.SIM_SLOT_NUMBER_1, String.valueOf(slotNumber_1));
				Utility.setPreferences(mContext, LocalConstants.SIM_SLOT_NUMBER_2, String.valueOf(slotNumber_2));
			} catch (Exception e) {
				//e.printStackTrace();
			}
		}
		...
		public ArrayList<String> getIMSIList(){
			...
			if(IMSI_1!=null && IMSI_2 !=null && IMSI_1.equalsIgnoreCase("")){
				...
				if(!TextUtils.isEmpty(IMSI2)){
					Utility.setPreferences(mContext, LocalConstants.SIM_SLOT_NAME_1, "1");
					Utility.setPreferences(mContext, LocalConstants.SIM_SLOT_NAME_2, "2");
					IMSI_1 = IMSI_2;
					IMSI_2 = IMSI2;
		...
		public void getDefaultSIMInfo(){
			...
			if(TextUtils.isEmpty(IMSI_1) || IMSI_1.equalsIgnoreCase(IMSI)){
				...
				NETWORK_TYPE_2 = Utility.getNetworkTypeName(telephony.getNetworkType());
			}
		...
		public void gettingAllMethodValues() {
			try {
				...
				if(IMSI_1!=null && IMSI_2 !=null && IMSI_1.equalsIgnoreCase("")){
					String IMSI2 = getMethodValue(telephonyClassName, m_IMSI,slotNumber_2+1);
					if(!TextUtils.isEmpty(IMSI2)){
					//	MeterPreferences.setStringPrefrence(mContext, LocalConstants.SIM_SLOT_NAME_1, "1");
					//	MeterPreferences.setStringPrefrence(mContext, LocalConstants.SIM_SLOT_NAME_2, "2");
						IMSI_1 = IMSI_2;
						IMSI_2 = IMSI2;
						slotNumber_1 = slotNumber_2;
						slotNumber_2 = slotNumber_2+1;
