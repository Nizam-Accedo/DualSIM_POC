...
public class DualSimManagerNew {
    ...
    public DualSimManagerNew(Context mContext) {
        if (IMEI_1 == null) {
    ...
    public static JSONObject getSimInfoJson(String simName, String networkType, boolean roaming, String operator, String imeiNumber) throws JSONException {
        JSONObject json = new JSONObject();
        json.put("name", simName);
        json.put("carrier", operator);
...
//    }
    ...
    public boolean isFirstSimActive() {
        if (IMSI_1 == null || IMSI_1.equalsIgnoreCase("null") || IMSI_1.equalsIgnoreCase("") || IMSI_1.length() == 0) {
            return false;
        } else
            return true;
    ...
    public boolean isSecondSimActive() {
        if (IMSI_2 == null || IMSI_2.equalsIgnoreCase("null") || IMSI_2.equalsIgnoreCase("") || IMSI_2.length() == 0) {
            return false;
        } else
            return true;
    ...
    /*public static boolean shouldShowSimInfoDialog() {
        boolean isTermConditionAccepted = PrefUtils.get(PrefKeys.SIM_INFO_CONDITION_ACCEPTED, false);
        int termConditionShowCount = PrefUtils.get(PrefKeys.SIM_INFO_REMAINING_SHOW_COUNT, MAX_SHOW_COUNT);
        long lastDisplayTime = PrefUtils.get(PrefKeys.SIM_INFO_LAST_SHOW_TIME, 0L);
        long timeDiff = System.currentTimeMillis()-lastDisplayTime;
        return (!isTermConditionAccepted && termConditionShowCount > 0 && timeDiff >= TIME_DIFF_15_DAYS);
    }

    public static void updateShowFlagsForNext() {
        int termConditionShowCount = PrefUtils.get(PrefKeys.SIM_INFO_REMAINING_SHOW_COUNT, MAX_SHOW_COUNT);
        termConditionShowCount--;
        PrefUtils.updatePreferences(PrefKeys.SIM_INFO_REMAINING_SHOW_COUNT, termConditionShowCount);
        PrefUtils.updatePreferences(PrefKeys.SIM_INFO_LAST_SHOW_TIME, System.currentTimeMillis());
    }

    public static void updateShowFlagsForNever() {
        PrefUtils.updatePreferences(PrefKeys.SIM_INFO_REMAINING_SHOW_COUNT, 0);
    }*/
}
